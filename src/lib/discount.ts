...
// 기획서 7장 표. 스펙 표의 5명 미만은 0%(파일럿 최소 3명).
...
export function buildDiscountTable(
  ...
  const table: DiscountTable = { peak: [], offpeak: [] };
  const bySlot: Record<TimeSlot, [number, number][]> = { peak: [], offpeak: [] };
  ...
  return { peak: bySlot.peak.length ? bySlot.peak : table.peak, offpeak: bySlot.offpeak.length ? bySlot.offpeak : table.offpeak };
}
